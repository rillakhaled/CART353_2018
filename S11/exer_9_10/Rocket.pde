...
class Rocket {
  ...
  //constructor
  Rocket(PVector l, DNA dna_, int totalRockets, PImage image) {
  ...
  void update() {
    ...
    // use get() on position to add the current position into our points array - we may want this later
    // to visualise the path to the target (if this hits the target)
  ...
  void display() {
    ...
    if (!hitTarget) {
      ...
      // if we have found the target, draw a line
      // joining all the points we'v been storing
      // in our points array
      // and everytime we do this, draw the line a bit more faintly
      strokeCol -= 2;
