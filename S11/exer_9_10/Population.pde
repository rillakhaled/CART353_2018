...
class Population {
  ...
  int generations;             // Number of generations
   ...
   Population(float m, int num) {
    ...
    // load the ghoul image just once
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      population[i] = new Rocket(position, new DNA(), population.length, ghoul);
    }
  }
  ...
  void reproduction() {
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      PVector position = new PVector(width/2,height+20);
      population[i] = new Rocket(position, child, population.length, ghoul);
    }
    generations++;
  }
