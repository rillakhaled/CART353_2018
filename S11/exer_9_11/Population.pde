...
class Population {
  ...
  int generations;             // Number of generations
   ...
   Population(float m, int num) {
    ...
    ghoul = loadImage("whiteGhoul.png");
    
    //make a new set of creatures
    for (int i = 0; i < population.length; i++) {
      PVector position = new PVector(width/2, height);
      population[i] = new Rocket(position, new DNA(), population.length, 6, 2, ghoul);
    }
  }
  ...
  void reproduction() {
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      // Get their genes
      DNA momgenes = mom.flow.getDNA();
      DNA dadgenes = dad.flow.getDNA();
      // Mate their genes
      ...
      PVector position = new PVector(width/2, height);
      population[i] = new Rocket(position, child, population.length, 6, 2, ghoul);
    }
    generations++;
  }
